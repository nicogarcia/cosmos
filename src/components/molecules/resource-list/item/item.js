...
const ResourceListItem = props => {
  ...
  if (props.image) {
    // TODO: We need some way to control the shape of the image, but we don't
    // want to leak every prop from Avatar into the ResourceListItem...
    image = <Avatar shape="square" image={props.image} />
  } else if (props.icon) {
    image = <Avatar shape="square" icon={props.icon} />
  }
