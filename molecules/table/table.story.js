'use strict';

var _react = require('react');

var _react2 = _interopRequireDefault(_react);

var _react3 = require('@storybook/react');

var _storyHelpers = require('../../_helpers/story-helpers');

var _ = require('../..');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var items = [{
  name: 'Isaac Asimov',
  born: 1920,
  died: 1992,
  image: 'data:image/jpeg;base64,'
}, {
  name: 'Ray Bradbury',
  born: 1920,
  died: 2012,
  image: 'data:image/jpeg;base64,'
}, {
  name: 'Philip K. Dick',
  born: 1928,
  died: 1982,
  image: 'data:image/jpeg;base64,'
}, {
  name: 'George Orwell',
  born: 1903,
  died: 1950,
  image: 'data:image/jpeg;base64,'
}];

(0, _react3.storiesOf)('Table', module).add('default', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'default' },
    _react2.default.createElement(
      _.Table,
      { items: items },
      _react2.default.createElement(_.Table.Column, { field: 'name', title: 'Name' }),
      _react2.default.createElement(_.Table.Column, { field: 'born', title: 'Born' }),
      _react2.default.createElement(_.Table.Column, { field: 'died', title: 'Died' })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('column as variable', function () {
  var columns = _react2.default.createElement(
    _react2.default.Fragment,
    null,
    _react2.default.createElement(_.Table.Column, { field: 'name', title: 'Name' }),
    _react2.default.createElement(_.Table.Column, { field: 'born', title: 'Born' }),
    _react2.default.createElement(_.Table.Column, { field: 'died', title: 'Died' })
  );

  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'default' },
    _react2.default.createElement(
      _.Table,
      { items: items },
      columns
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('explicit widths', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'default' },
    _react2.default.createElement(
      _.Table,
      { items: items },
      _react2.default.createElement(_.Table.Column, { field: 'name', title: 'Name', width: '70%' }),
      _react2.default.createElement(_.Table.Column, { field: 'born', title: 'Born' }),
      _react2.default.createElement(_.Table.Column, { field: 'died', title: 'Died' })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('cell renderer', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'default' },
    _react2.default.createElement(
      _.Table,
      { items: items },
      _react2.default.createElement(
        _.Table.Column,
        { field: 'image', width: '65px' },
        function (item) {
          return _react2.default.createElement(_.Avatar, { image: item.image, size: 'large' });
        }
      ),
      _react2.default.createElement(_.Table.Column, { field: 'name', title: 'Name', width: '65%' }),
      _react2.default.createElement(_.Table.Column, { field: 'born', title: 'Born' }),
      _react2.default.createElement(_.Table.Column, { field: 'died', title: 'Died' })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('sorting', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'default' },
    _react2.default.createElement(
      _.Table,
      { items: items },
      _react2.default.createElement(
        _.Table.Column,
        { field: 'image', width: '65px' },
        function (item) {
          return _react2.default.createElement(_.Avatar, { image: item.image, size: 'large' });
        }
      ),
      _react2.default.createElement(_.Table.Column, { field: 'name', title: 'Name', width: '65%' }),
      _react2.default.createElement(_.Table.Column, { field: 'born', title: 'Born', sortable: true }),
      _react2.default.createElement(_.Table.Column, { field: 'died', title: 'Died', sortable: true })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('initial sort props', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'default' },
    _react2.default.createElement(
      _.Table,
      { items: items, sortOn: 'died', sortDirection: 'desc' },
      _react2.default.createElement(
        _.Table.Column,
        { field: 'image', width: '65px' },
        function (item) {
          return _react2.default.createElement(_.Avatar, { image: item.image, size: 'large' });
        }
      ),
      _react2.default.createElement(_.Table.Column, { field: 'name', title: 'Name', width: '65%' }),
      _react2.default.createElement(_.Table.Column, { field: 'born', title: 'Born', sortable: true }),
      _react2.default.createElement(_.Table.Column, { field: 'died', title: 'Died', sortable: true })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('stressed', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'stressed - 4 columns with 119 characters per row' },
    _react2.default.createElement(
      _.Table,
      {
        items: [{
          data: 'Loremipsumdolorsitametconsecteturadipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
        }, {
          data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
        }, {
          data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
        }, {
          data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
        }]
      },
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 1', width: '65px' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 2', width: '150px' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 3', width: '65px' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 4', width: '65px' })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('stressed with truncating', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'stressed - 7 columns with 119 characters per row' },
    _react2.default.createElement(
      _.Table,
      {
        items: [{
          data: 'Loremipsumdolorsitametconsecteturadipiscing Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
        }, {
          data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
        }, {
          data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
        }, {
          data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
        }]
      },
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 1', width: '150px', truncate: true }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 2', width: '10%', truncate: true }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 3', width: '20%', truncate: true }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 4', width: 'auto', truncate: true })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('with no items', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'no items' },
    _react2.default.createElement(
      _.Table,
      {
        items: [],
        emptyState: _react2.default.createElement(
          _.EmptyState,
          {
            helpUrl: 'auth0.com',
            icon: 'users',
            title: 'Users',
            action: {
              icon: 'plus',
              label: 'Create one manually',
              handler: function handler() {
                /*...*/
              }
            }
          },
          'You don\'t have any users in your tenant at the moment'
        )
      },
      _react2.default.createElement(
        _.Table.Column,
        { field: 'image', width: '65px' },
        function (item) {
          return _react2.default.createElement(_.Avatar, { type: 'user', image: item.image });
        }
      ),
      _react2.default.createElement(_.Table.Column, { field: 'name', title: 'Name', width: '30%' }),
      _react2.default.createElement(_.Table.Column, { field: 'country', title: 'Country' }),
      _react2.default.createElement(_.Table.Column, { field: 'goals', title: 'Goals', sortable: true }),
      _react2.default.createElement(_.Table.Column, { field: 'assists', title: 'Assists', sortable: true })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('loading state with data', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'loading' },
    _react2.default.createElement(
      _.Table,
      {
        loading: true,
        items: [{
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }, {
          data: 'Lorem ipsum dolor sit amet.'
        }]
      },
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 1' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 2' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 3' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 4' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 7' })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('loading state without data', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'loading' },
    _react2.default.createElement(
      _.Table,
      { loading: true, items: [] },
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 1' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 2' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 3' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 4' }),
      _react2.default.createElement(_.Table.Column, { field: 'data', title: 'Field 7' })
    )
  );
});

(0, _react3.storiesOf)('Table', module).add('with buttons', function () {
  return _react2.default.createElement(
    _storyHelpers.Example,
    { title: 'with buttons' },
    _react2.default.createElement(
      _.Table,
      {
        items: [{
          name: 'Harry Kane',
          goals: 6,
          assists: 0,
          country: '🇬🇧',
          image: 'https://twitter-avatar.now.sh/HKane'
        }, {
          name: 'Romelu Lukaku',
          goals: 4,
          assists: 1,
          country: '🇧🇪',
          image: 'https://twitter-avatar.now.sh/Romelu_lukaku9'
        }, {
          name: 'Antoine Griezmann',
          goals: 4,
          assists: 2,
          country: '🇫🇷',
          image: 'https://twitter-avatar.now.sh/AntoGriezmann'
        }, {
          name: 'Ivan Perišić',
          goals: 3,
          assists: 1,
          country: '🇭🇷',
          image: 'https://twitter-avatar.now.sh/ivanperisic44'
        }],
        onRowClick: function onRowClick(evt, item) {
          return alert(item.name + ' was clicked!');
        }
      },
      _react2.default.createElement(
        _.Table.Column,
        { field: 'name', title: 'Name', width: '30%' },
        function (item) {
          return _react2.default.createElement(_.AvatarBlock, { type: 'user', image: item.image, title: item.name, size: 'compact' });
        }
      ),
      _react2.default.createElement(_.Table.Column, { field: 'country', title: 'Country' }),
      _react2.default.createElement(_.Table.Column, { field: 'goals', title: 'Goals' }),
      _react2.default.createElement(
        _.Table.Column,
        { field: 'assists', title: 'Assists', width: '5%' },
        function (item) {
          return _react2.default.createElement(_.Button, { appearance: 'default', icon: 'emails', href: 'https://auth0.com' });
        }
      )
    )
  );
});